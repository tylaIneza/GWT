          ...
          <div className="card overflow-hidden divide-y divide-gray-800">
            {challenges.map((c: any, i: number) => (
              <Link key={c.id} href={`/challenges/${c.id}`}
                className="flex items-center gap-4 px-5 py-4 hover:bg-gray-800/30 transition-colors group">
                <span className="text-gray-600 text-sm w-6 shrink-0">{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-white group-hover:text-green-300 transition-colors truncate">{c.title}</p>
