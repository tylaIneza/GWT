  ...
  async initiateDeposit(userId: string, amount: number, phone: string, provider: 'mtn'|'airtel') {
    ...
    await this.db.execute(
      `INSERT INTO transactions
         (id, wallet_id, user_id, type, amount, balance_before, balance_after, status, reference, payment_provider, provider_reference)
       VALUES (?,?,?,'deposit',?,?,?,'pending',?,?,?)`,
      [uuid(), w.id, userId, amount, w.balance, w.balance, reference, provider, providerRef],
    );
